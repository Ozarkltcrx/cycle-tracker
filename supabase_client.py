...
def export_and_reset_bag_counts(email_to: str = "acheeley@ozarkltcrx.com", reset: bool = True) -> str:
    """Export bag counts using the new openpyxl-based export module."""
    from bag_count_export import export_bag_counts
    return export_bag_counts(email_to=email_to, reset=reset)


# ── Historical Week Archives ────────────────────────────────────────────────

HISTORY_FILE = DATA_DIR / "tracking_history.json"

def load_tracking_history() -> dict:
    """Load historical tracking data by week.
    ...
    Structure:
    {
        "2026-W13": {
            "cycle_team_tracking": {...},
            "dollar_tracking": {...},
            "archived_at": "2026-03-31T00:00:00"
        },
        "2026-W12": {...},
        ...
    }
    ...
    if _USE_SUPABASE:
        try:
            resp = _supabase_client.table("tracking_state").select("value").eq("key", "tracking_history").execute()
            if resp.data:
                return resp.data[0]["value"]
        except Exception:
            pass
...
def save_tracking_history(history: dict) -> None:
    """Save historical tracking data."""
    if _USE_SUPABASE:
        try:
            _supabase_client.table("tracking_state").upsert(
                {"key": "tracking_history", "value": history},
                on_conflict="key",
            ).execute()
            return
        except Exception:
...
def archive_week_data(week_key: str, cycle_data: dict, dollar_data: dict) -> None:
    """Archive a week's tracking data to history.
    
    Called during week rollover to preserve previous week's data.
    Keeps last 12 weeks of history.
    """
    if not week_key or (not cycle_data and not dollar_data):
        return
    ...
    history[week_key] = {
        "cycle_team_tracking": cycle_data,
        "dollar_tracking": dollar_data,
        "archived_at": datetime.now().isoformat(),
    }
    
    # Keep only last 12 weeks (prune older)
    if len(history) > 12:
...
def get_available_history_weeks() -> list[str]:
    """Get list of weeks with archived data, most recent first."""
    history = load_tracking_history()
    return sorted(history.keys(), reverse=True)


def get_week_history(week_key: str) -> dict | None:
